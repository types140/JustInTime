...
(function() {
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body" id="jit_body">
                    <div class="form-check" id="jit_check_1">
                        <input class="form-check-input" type="checkbox" value="" id="jit_own_missions" name="jit_own_missions">
                        <label class="form-check-label" for="jit_own_missions"> Eigene Einsätze einbeziehen?</label>
                    </div>
                    <hr>
                    <div id="jit_check_ids">Folgende Fahrzeuge werden geprüft: </div>
                    <hr>
                    <div id="jit_show_checkboxes">Auswahl anzeigen</div>
    ...
    $.each(arrFahrzeugDaten, function(i,e){
        $("#jit_ids_ul").append('<li><input class="" type="checkbox" value="' + e.id + '" id="jit_id_checkboxes' + e.id + '" name="jit_ids[]"><label class="checkbox-item" for="jit_id_checkboxes' + e.id + '"> ' + e.name + '</label></li>');

        $.each(car_ids, function(i2,e2){
            if(i == e2) {
                $("#jit_id_checkboxes" + e2).prop("checked", true);
                $("#jit_check_ids").append("<br/>" + e.name);
            }
        });
    });
    ...
    function jit_main(){
        ...
        $(".vehicle_select_table_tr").each(function(){
            $("#mission_countdown_remaining").removeClass();
            ...
            if(car_id != 0) {
                if(time_remain > 60) {
                    $("#mission_countdown_remaining").addClass("alert alert-success");
                } else if(time_remain <= 60 && time_remain > 0) {
                    $("#mission_countdown_remaining").addClass("alert alert-warning");
                } else if(time_remain < 0){
                    $("#mission_countdown_remaining").addClass("alert alert-danger");
        ...
        if(isNaN(mission_secs)) {
            $("#mission_countdown_remaining").removeClass().addClass("alert alert-info");
    ...
    $(".calculateTime").click(function(){ setTimeout(function(){ jit_main }, 1000); });
